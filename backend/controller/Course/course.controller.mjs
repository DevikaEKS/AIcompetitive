...
export const getSubmodulesByCourseAndModule = (req, res) => {
  ...
  if (!courseid || !moduleid) {
    return res
      .status(400)
      .json({ error: "Course ID and Module ID are required." });
  }

  // SQL query to fetch submodules along with the question count for each submodule
  const query = `
      SELECT 
          ...
          submodule.courseid,
          submodule.moduleid,
          COUNT(quiz_text.id) AS question_count
      FROM submodule
      LEFT JOIN quiz_text 
          ON submodule.submodule_id = quiz_text.submoduleid
      WHERE submodule.courseid = ? AND submodule.moduleid = ?
      GROUP BY submodule.submodule_id
  `;

  db.query(query, [courseid, moduleid], (err, results) => {
    if (err) {
      console.error("Error fetching submodules and question counts:", err);
    ...
    return res.status(200).json({
      message: "Submodules and question counts fetched successfully!",
      results,
    });
  });
};
...
export const getSidebarData = (req, res) => {
  const query = "SELECT * FROM quiz_type"; // Adjust the query as per your table structure
